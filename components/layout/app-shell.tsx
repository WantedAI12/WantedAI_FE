    ...
    <main className="min-h-screen bg-background text-foreground">
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-64 border-r border-sidebar-border bg-sidebar px-4 py-5 lg:flex lg:flex-col">
        <Link href="/" className="flex items-center gap-3 px-3">
          ...
          <div>
            <p className="font-semibold tracking-tight">Perfumery AI</p>
            <p className="text-xs text-muted-foreground">R&amp;D Workspace</p>
          </div>
        </Link>
        <nav aria-label="주요 메뉴" className="mt-9 space-y-1">
          {menu.map(({ label, href, icon: Icon }) => {
            ...
            return (
              <Link
                key={href}
                href={href}
                className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition-colors ${active ? 'bg-sidebar-accent font-medium text-sidebar-accent-foreground' : 'text-muted-foreground hover:bg-sidebar-accent/60 hover:text-foreground'}`}
              >
        ...
        <div className="mt-auto rounded-2xl border border-sidebar-border bg-white/70 p-3.5">
      ...
      <section className="lg:pl-64">
        <header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b bg-background/90 px-5 backdrop-blur md:px-8">
          <Link href="/" className="flex items-center gap-2 lg:hidden">
            ...
            <span className="font-semibold">Perfumery AI</span>
          </Link>
          <div className="relative hidden w-full max-w-sm md:block">
            <Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
            <Input
              aria-label="통합 검색"
              placeholder="프로젝트, 조향식, 원료 검색"
