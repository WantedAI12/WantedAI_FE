...
function Composition() {
  ...
  return <div className="wf-formula-cols"><div><h2 className="wf-panel-title">향료 원액 배합비</h2><div className="wf-mix-card"><div className="wf-bars">{rows.map(([name,width])=><div className="wf-bar-row" key={name}><span>{name}</span><div className="wf-bar-track"><div className="wf-bar-fill" style={{width:`${width * 1.86}%`}} /></div><span>{width.toFixed(1)}%</span></div>)}</div></div><section className="wf-product-content"><h2 className="wf-panel-title">완제품 함량</h2>{[['적용 농도','10%'],['배치 기준','100kg'],['향료 함량','10kg'],['기타 원료','90kg']].map(([label,value],index)=><div className={index===2?'is-highlighted':''} key={label}><span>{label}{index===3&&<i>⌄</i>}</span><b>{value}</b></div>)}</section></div><div><h2 className="wf-panel-title">조향식 정보</h2><div className="wf-info-card">{[['목표 일치도','96%'],['예상 원가','₩4,180 / 100ml'],['사용 농도','0.2~0.5%'],['제품 유형','바디로션'],['지속성 예측','8.6h']].map(([label,value])=><div className="wf-info-row" key={label}><span>{label}</span><b>{value}</b></div>)}</div></div></div>;
}

function Performance() {
  const series = [
    {name:'Citrus', color:'#858585', points:'20,100 145,42 390,64 625,82 885,122'},
    {name:'Woody', color:'#969696', points:'20,112 145,78 390,58 625,76 885,94'},
    {name:'Amber', color:'#000', points:'20,150 145,132 390,148 625,146 885,164'},
    {name:'Musk', color:'#d9d9d9', points:'20,180 145,180 390,180 625,180 885,180'},
  ];
  return <div className="wf-performance"><h2 className="wf-panel-title">시간에 따른 향 변화</h2><div className="wf-performance-chart"><svg viewBox="0 0 930 220" aria-labelledby="performance-chart-title"><title id="performance-chart-title">시간에 따른 Citrus, Woody, Amber, Musk 향 변화 그래프</title><line x1="20" y1="20" x2="20" y2="180"/><line x1="20" y1="180" x2="900" y2="180"/><line className="grid-line" x1="20" y1="100" x2="900" y2="100"/><text x="-2" y="24">100</text><text x="4" y="104">50</text><text x="9" y="184">0</text>{series.map(item=><g key={item.name} style={{color:item.color}}><polyline points={item.points}/>{item.points.split(' ').slice(1).map((point,index)=>{const [cx,cy]=point.split(',');return <circle key={index} cx={cx} cy={cy} r="7"/>})}</g>)}{[['0h',20],['1h',145],['4h',390],['8h',625],['12h',885]].map(([label,x])=><text className="time-label" key={label} x={x} y="211" textAnchor="middle">{label}</text>)}</svg><div className="wf-chart-legend">{series.map(item=><span key={item.name}><i style={{background:item.color}}/>{item.name}</span>)}</div></div><div className="wf-performance-cards"><div className="wf-performance-summary"><span>사용 농도</span><b>10%</b><span>지속성 예측</span><b>약 8 h</b><span>예측 상태</span><b>적용 범위 내</b></div><div className="wf-scent-ratios">{[['Citrus','22%'],['Woody','38%'],['Amber','26%'],['Musk','14%']].map(([label,value])=><div key={label}><span>{label}</span><b>{value}</b></div>)}</div></div><div className="wf-calculation-note"><b>ⓘ&nbsp; 계산조건</b><span>적용 농도 10%</span><span>베이스 : 바디로션</span><span>사용조건: 해당 조건 기준</span></div></div>;
}
